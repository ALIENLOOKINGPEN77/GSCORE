  ...
  try {
    ...
    const calculateTotalsByUnit = (movementsList) => {
      ...
      movementsList.forEach(m => {
        const unit = m.unidadDeMedida || 'Sin unidad';
        if (!unitTotals[unit]) {
          unitTotals[unit] = 0;
        }
        unitTotals[unit] += Math.abs(m.qty);
      });
      return unitTotals;
    };
      ...
      return Object.entries(unitTotals)
        ...
        .map(([unit, total]) => `
          <div style="margin-bottom: 5px; padding-left: 20px;">
            <strong>${unit}:</strong> ${prefix}${total.toLocaleString('es-ES')}
          </div>
        `).join('');
    };
    ...
    // UPDATED: Font size set to 10px (with 9px for description/location and 8px for ID)
    ...
    // Pagination settings
    // UPDATED: Adjusted to 15 to fit 10px rows comfortably on A4
